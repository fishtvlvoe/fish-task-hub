...
### 搬移地址簿用固定路徑檔案，不寫進 PR 說明

Fish 提出的真實問題：資料夾搬移後，其他 Agent／CLI／腳本讀到舊路徑會找不到東西，若把「這次搬了什麼」只寫在 PR 說明或對話紀錄裡，之後的 Agent 不會主動去翻歷史 PR，等於沒人看得到。改用固定路徑的 `docs/folder-moves.json` 當 append-only 地址簿，加上舊路徑留 `.moved-to` 指標檔雙保險：就算 Agent 沒看規則、直接 `cd` 到舊路徑，也會看到指標檔案本身，不需要事先知道地址簿存在。`AGENTS.md`／`CLAUDE.md` 也必須明講「找不到路徑先查地址簿」，讓新接手的 Agent 第一次讀工作區規則就知道這個機制存在。

替代方案（否決）：只寫進 PR 說明或 commit message。否決理由：PR／commit 是給人看一次的歷史紀錄，Agent 不會主動搜尋所有歷史 PR 才能確認一個路徑是否被搬過；地址簿必須是「當下讀取工作區就會看到」的固定檔案，不是需要額外搜尋才找得到的歷史紀錄。

替代方案（否決）：只留 `.moved-to` 指標檔，不做地址簿。否決理由：指標檔只能解決「剛好走到舊路徑」的情況，無法讓 Agent 一次看到「這個工作區到目前為止總共搬過哪些東西」的完整歷史，也無法在原始碼審查或除錯時回溯特定日期做過的搬移。兩者互補，缺一不可。
